	...
	<body>	
		...
		<div class="main_visual ">   
			...
			<div class="main_image">
				<ul>
                    <?php if(!empty($data)) { foreach ($data as $values) {  if($values->recommend_type == 4) {//var_dump($values);?>
					<li>
						<a class="img_1" href="<?php echo $values->target_url ;?>"><img src="<?php echo $values->thumb ;?>"></a>
					</li>
					<?php }else{ //var_dump($values);?>
                        <li>
                        <a href="<?php echo \Yii::$app->urlManager->createUrl(['life/goodsdetails','goodsId' =>$values->target_id])?>"><img src="<?php echo $values->thumb ;?>"></a>
                        </li>
                  <?php  }
                    } }?>
				</ul>
		...
		<div class="locContent">
			<div class="data-width"> 
				<div class="hot-sale">
					<div class="hot-title">
						<h3 class="one">爆款抢购</h3>
					</div>
					<div class="hot-main fix">
						<ul>
                            <?php if(!empty($data1)) { foreach ($data1 as $values) { //var_dump($values);die;?>
							<li class="hot-list miaosha">

                                    <?php if($values->recommend_type == 2){ ?>
                                <a href="<?php echo \Yii::$app->urlManager->createUrl(['activity/hot','activity_id' =>$values->target_id])?>">
                                    <? }elseif($values->recommend_type == 4){ ?>
                                    <a href="<?php echo $values->target_url?>">
                                    <?php } ?>
									<img src="<?php echo $values->thumb ;?>" alt="" />
								</a>
				...
				<!--log展示-->
				<div class="hot-log">
                    <?php if(!empty($data2)) { foreach ($data2 as $values) {  if($values->recommend_type == 4) { //var_dump($values);die;?>
					<a href="<?php echo $values->target_url ?>">
						<img src="<?php echo $values->thumb ;?>" alt="" />
					</a>
					<?php }else{ ?>
                        <a href="<?php echo \Yii::$app->urlManager->createUrl(['life/goodsdetails','goodsId' =>$values->target_id])?>">
                            <img src="<?php echo $values->thumb ;?>" alt="" />
                        </a>
                  <?php  }
                    } } ?>
		...
		//三级菜单的选项卡
			$(function(){	
			    $('.channel-items').find(".item").mouseover(function(){
			    	...
			    	$(this).siblings().find('.item-l').find('a').css({color:'#fff'})
			    	$(this).find('.item-l').find('a').css({color:'#282828'});
			    	$(this).find('.channel-tabs .tabs').css({display:"block"});
			    	$(this).siblings().find('.channel-tabs .tabs').css({display:"none"});
			    })
			     $('.channel-items').find(".item").mouseout(function(){
			     	$(this).removeClass("item-active");
			     	$(this).find('.item-l').find('a').css({color:'#fff'})
			     	$(this).find('.channel-tabs .tabs').css({display:"none"});			     	
				 })	
			})					
         //切换商家和团购	
		$(function(){
